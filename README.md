...
- 记忆闪打内置 48,755 条成语和 3,248 条唐诗诗句，并复用普通练习中的英语单词、JavaScript、Python、HTML、CSS 术语；查看与输入阶段都有动画时钟和倒计时，输入后按 Enter 可立即进入下一题，超时则自动提交切题。
- 记忆闪打按难度提供 5/3/1 秒查看时间和 15/10/6 秒输入时间，并有可关闭的倒计时、超时和切题提示音。
...
- `ohmytype_memory_sound`：记忆闪打倒计时与切换音效是否开启。
